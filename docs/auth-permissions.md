...
:::note
Per-env grants are only consulted on routes that resolve their env from the path — records,
types, indexes, TTL, views, subscriptions and env admin. The files, queue, FTS and functions
routes currently check **global** `permissions` only, so a per-env `files:read` /
`queue:work` / `functions:run` / `search:query` grant does not open them — grant those globally.
:::
