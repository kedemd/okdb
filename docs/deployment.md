...
With the CLI, the serving node takes the same flags from its config file (they apply to the
single process and to every cluster worker; see [HTTP Clustering](./http-cluster.md#role-flags-in-kdbconfig)):

```json
// front.kdbconfig — okdb --config front.kdbconfig --path /srv/okdb --port 8080
...
Why it matters: lmdb-js (3.5.x) has a bug where opening an LMDB environment while another
process commits to it can rewind the shared transaction id, which later shows up as native
crashes (`SIGSEGV`, `MDB_PROBLEM`) or a lost commit. okdb works around the worst case itself:
while a store is being **initialised** (an env it opens at boot has no data file yet), `open()`
takes a cross-process **boot lock**, so several processes starting on a brand-new path boot one
at a time. A dead or stale holder is taken over; a waiter that exceeds
`OKDB_BOOT_LOCK_TIMEOUT_MS` (default 60 000) logs a warning and proceeds unserialised. On an
established store the lock is not taken, so opening a process alongside a peer that is actively
writing — and runtime `createEnvironment()` / `removeEnvironment()` — remain exposed until the
lmdb-js issue is fixed upstream; staggered starts keep that window small.
...
`okdb backup <dir>` / `db.migrate.backup(dir)` take a hot backup (LMDB snapshot copy of every
env + time-machine history + file blobs; full-text indexes rebuild on first open) into a new,
empty directory outside the store; `okdb restore <dir> --path <target>` puts it back. Details
and caveats: [migration.md → Hot backup and restore](migration.md#hot-backup-and-restore).

Backups requested **remotely** (`POST /api/system/backup`, MCP `system_backup`) can only write
under the server's backup root — the `backup.root` option (`.kdbconfig`: `"backup": { "root":
"/mnt/backups/okdb" }`), default `<store>-backups` next to the store. Put the root on a volume
with room for a full copy of the store; the caller only chooses a directory name inside it.
