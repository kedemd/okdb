...
| `bus: true \| false`                   | **removed** — auto-enabled whenever the shmbuf native binding is present (pokes throttled, see [sync](sync.md#udp-discovery-bus))        | `BUS_OPTION_REMOVED`            |
...
**Why these are safe to remove:** `bus` is load-bearing and derived (native shmbuf ⇒ on);
