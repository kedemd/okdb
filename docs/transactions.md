...
All errors extend `OKDBError` and carry a `code` property. The error classes are not exported
from the package — branch on `err.code`:
...
The `/transaction` endpoint is the right tool for bulk ingest over HTTP. It accepts an array of operations and commits them all in a single LMDB transaction. The route is `POST /api/transaction` for the default env, or `POST /api/env/<env>/transaction` for any env (see [HTTP API](./http-api.md#environment-prefix)). The target types must already be registered.

```bash
curl -X POST http://localhost:8080/api/env/default/transaction \
...
The body may be the `{ "operations": [...] }` wrapper shown above or the bare operations array
(`[ { "action": "put", … }, … ]`); both are validated the same way.

Responds `200` with `data: null` on success. If any operation fails (validation, unknown type,
`ifVersion` mismatch, `remove` of a missing key, …), the entire transaction is aborted and
nothing is written.
