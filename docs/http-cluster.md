...
```
                ...
                │             engines:false, compaction:false)   │
                ...
                │  • no HTTP server; does NOT process            │
                └───────────────┬──────────────────────────────┘
                                │ forks N
        ┌───────────────┬───────┴───────┬───────────────┐
        ▼               ▼               ▼               ▼
...
- **Supervisor = passive.** It opens a passive OKDB (`processors:false`, `engines:false`,
  ...
  registry. It runs no HTTP server and never processes — but on Linux/macOS it **does** sit in
  the connection-accept path (see [Connection distribution](#connection-distribution)).
...
okdb uses Node `cluster`'s default scheduling. On Linux and macOS that is **round-robin
(`SCHED_RR`)**: the supervisor process owns the listen socket, `accept()`s every **new**
connection on its own event loop, and hands the socket to a worker over IPC. Consequences:

- Workers are isolated from each other and from the supervisor for **established** connections —
  requests on a kept-alive connection go straight to its worker.
- Workers are **not** isolated from the supervisor for **new** connections: if the supervisor's
  loop is blocked, every new connection waits until it frees up (measured: a 1.5 s busy-loop in
  the supervisor delayed a new connection by ~1.4 s with 4 idle workers). The supervisor is kept
  thin for this reason — it only forks/respawns workers, relays IPC, and publishes registry rows.
- Distribution is even (strict rotation).

Operators who prefer the OS to distribute connections can set Node's own
`NODE_CLUSTER_SCHED_POLICY=none` (`SCHED_NONE`) in the `okdb` process environment: each worker
`accept()`s directly, so a busy supervisor no longer delays new connections — at the cost of
uneven distribution (measured on Linux with 4 idle workers: 42–209 of 400 serial connections per
worker; under load the kernel favours whichever workers are waiting in `accept()`, which can
skew further). Windows always uses `SCHED_NONE`.
...
### Bind address: `http.host`

By default the CLI listens on every interface. `http.host` (next to `http.workers`) binds the API
to one address or an array of them — in single-process mode and in every cluster worker; the
registry's `listenAddr` shows it. There is no `--host` flag.

```json
// .kdbconfig — loopback only (e.g. behind a local reverse proxy)
{ "http": { "host": "127.0.0.1" } }
```

### Role flags in `.kdbconfig`

The config file also takes the constructor role flags as top-level booleans — the same names and
semantics as `new OKDB(path, { processors, engines, compaction })` (see
[Roles & Deployment](./deployment.md)). They apply to the single-process node and to **every**
forked worker; absent keys keep the constructor default (`true`), and a non-boolean value is a
startup error. (The supervisor is always passive, whatever the config says.) There is no CLI
flag for them.

```json
// .kdbconfig — a passive serving node: serves HTTP, never claims a lease, never compacts
{ "processors": false, "engines": false, "compaction": false }
```
...
- **Processing progress is per-worker; the cluster-wide view is the durable registry.** There is
  ...
  **its own** SSE. The cluster-wide picture is **read** from the durable `~processes` registry:
  ...
  stamps it onto the worker's registry row, and any node serves `/api/processors/status` (and
  its deprecated alias `/api/processing/status`) and `/api/cluster/status` from that shared registry. So a processing
...
if (cluster.isPrimary) {
    ...
    const supervisor = new OKDB(dbPath, { processors: false, engines: false, compaction: false });
    await supervisor.open();
    for (let i = 0; i < 4; i++) cluster.fork();
} else {
    // Each worker = an identical CAPABLE node: serves HTTP AND processes (claims leases,
    // drains them on its own loop in bounded quanta). The lease distributes processing across them.
    const w = new OKDB(dbPath, { processors: true });
    await w.open();
    w.http.listen(port); // shared socket via Node cluster; bus is on (native shmbuf)
}
```
