...
`resourceLimits`, with `terminate()` as the timeout/wedge watchdog. One sandbox per process per
memory tier (in the common case — every function at the default `memoryMb` — exactly one thread),
...
- **Run deadline** — `runtime.timeoutMs` on the stored function (default 5 000 ms) drives
  `ctx.signal` and the sandbox watchdog (`FUNCTION_TIMEOUT`). A single call can override it with
  `functions.run(name, payload, { timeoutMs })`; the value must be a positive finite number
  (`FUNCTION_INVALID_TIMEOUT` otherwise) and is capped at `OKDB_FN_MAX_TIMEOUT_MS` (default
  600 000 ms), since a running call holds a sandbox slot for its whole deadline.
- **Compile bound** — evaluating the stored source to obtain the function (at create/update and
  in the sandbox) is bounded by a wall-clock watchdog, `OKDB_FN_COMPILE_TIMEOUT_MS` (default
  1 000 ms). A valid source is a bare arrow expression and evaluates in well under a millisecond;
  the bound only stops a source that smuggles top-level work.
...
- `files` — files feature for this env: `upload`, `stream`, `read`, `remove`, `get`, `listPath`. `read(id)` resolves the whole blob as a Buffer (rejects with `status: 404` for an unknown id). A function's return value is structured-cloned out of the sandbox and stored as JSON, so convert bytes before returning them — e.g. `buf.toString('utf8')` or `buf.toString('base64')`; a raw Buffer comes back as an index-keyed object.
...
| `meta.runnerId` | Executor id (e.g. `'sandbox-12345'` — the sandbox thread's host pid) |
