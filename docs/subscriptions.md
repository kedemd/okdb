...
All routes are under `/api/env/<dataEnv>/subscriptions`, where `<dataEnv>` is the real data
environment whose changes you want to watch. For the default env the `/env/<dataEnv>` segment
can be dropped: `/api/subscriptions`.
...
`GET /api/env/<dataEnv>/subscriptions` opens an SSE stream (`?type=` names the type to watch,
`?filter=` is a JSON-encoded filter, `?sub=` reuses a session id):
...
2. **Signals** — every change matching the session's interests/filter arrives as:
    ...
    view. `op` is `put` or `remove`; `clock` is the per-`(env,type)` changelog clock. A signal
    produced by the filter's enter-discovery also carries `"hint": "enter-candidate"`.
...
- **Explicit key interests** — for the keys the client currently shows. Register on enter,
  deregister on leave. Best for small/specific sets — no cap is enforced, but push
  large/dynamic sets to a filter instead.
...
```
POST /api/env/<dataEnv>/subscriptions/<id>/interests   { "add": [{ "type": "Order", "key": "o-42" }] }
PUT  /api/env/<dataEnv>/subscriptions/<id>/interests   { "interests": [{ "type": "Order", "key": "o-42" }] }
POST /api/env/<dataEnv>/subscriptions/<id>/filter      { "filter": { "status": "open" } }
DELETE /api/env/<dataEnv>/subscriptions/<id>
```

Unknown `sessionId` → `404` (`SESSION_NOT_FOUND`).
...
// PUT-replaces its interest set on (re)connect / resync. Responses use okdb's
// { data, error, meta } envelope.
function subscribe(dataEnv, { type, filter } = {}) {
    const base = `/api/env/${dataEnv}`;
    ...
    const es = new EventSource(`${base}/subscriptions?${qs}`);

    // Re-declare the full interest set (PUT-replace, not append — race-safe on reconnect).
    async function resync() {
        const { data: rows } = await fetch(`${base}/type/${type}/query`, {
            method: 'POST',
        ...
        view.clear();
        for (const d of rows) view.set(d.key, d.value);

        await fetch(`${base}/subscriptions/${sessionId}/interests`, {
            method: 'PUT',
            headers: { 'content-type': 'application/json' },
    ...
    es.addEventListener('signal', async (e) => {
        ...
        // A missing key answers 200 with data: { key } and no `value`.
        const { data: doc } = await fetch(`${base}/type/${type}/item/${encodeURIComponent(key)}`).then((r) => r.json());
    ...
    function control(method, body) {
        return fetch(`${base}/subscriptions/${sessionId}/interests`, {
            method,
            headers: { 'content-type': 'application/json' },
...
- **`session`** — `{ sessionId, dataEnv, type, filter?, version, createdAt, expiresAt }`,
  with per-doc TTL on `expiresAt`, and an index on `[dataEnv, type]` so filter (enter)
  evaluation reads only the sessions on the changed type.
- **`interest`** — `{ id, sessionId, dataEnv, type, key }`, with a `ref sessionId → session`,
  `onDelete: cascade`, and a reverse index on `[dataEnv, type, key]` for O(interested
  sessions) routing.

Routing is lazy: a process routes changes only while it holds at least one open subscription
connection (signals are only ever delivered to connections on the process that holds them).
An embedded instance that never serves subscriptions does no per-change subscription work,
and no change-feed is started on its behalf.
