...
### Persisted history — `GET /api/system/logs`

Every non-debug entry is also written (batched, ≤1 commit/s) to the `~log` env's
`~log_entry` type, kept for `log.retainDays` (default 2) via TTL. The admin
**System → Logs → History** tab reads it through:

```
GET /api/system/logs?from=<ms>&to=<ms>&level=error,warn&feature=queue&node=<id>&limit=50&cursor=<next_cursor>
→ { data: { entries: [{ key, ts, level, msg, feature, node, context }], next_cursor } }
```

Entries come newest first. The read is **bounded**: it walks a ts-ordered index in reverse
(the `[level, ts]` / `[node, ts]` / `[feature, ts]` index when that filter has a single
value) and examines at most 5 000 rows per call, so a sparse filter can return a short — even
empty — page with a non-null `next_cursor`. Keep passing `next_cursor` back as `cursor`
until it is `null`. The cursor is opaque (`"<ts>:<key>"`); a bare numeric cursor from older
callers still means "strictly older than this ts". `limit` defaults to 50, max 200.
