...
`replaceMember()` is the supported _parameter_ mutation: it swaps one member for another **existing engine of the same type**, re-points sibling members whose `config.<role>` referenced the old name (the indexer's and search engine's `embedder` field), restarts them, and rewrites the record. It does not re-embed — follow with `rebuild()` (UI: "Re-embed all") when the new member produces different output; replacing the embedder with one of another provider/model/dims marks the pipeline `stale` (new embeds held) until you do. `remove()` deletes env-owned members unless another pipeline record in the env still references them.
...
Embeddings pipelines also report `stale` + `vector_space` (independent of `health`): `stale: true`
means the embedder's provider/model/dims changed since the stored vectors were embedded — search
serves the old vectors with a warning, new embeds are held, and `rebuild()` ("Re-embed all")
clears it. See [Embeddings → Stale pipelines](./embeddings.md#stale-pipelines-embedder-model-changed).
